...
static float colorLowCut  = -0.8f;   // bottom 15%
static float colorHighCut =  0.9f;   // top 25%
static float* geo = 0;
...
static void buildTerrain()
{
    for (int gz = 0; gz < GRID; ++gz) {
        for (int gx = 0; gx < GRID; ++gx) {
            ...
            // Extremely mild edge falloff only, not a bowl
            float cx = fx - 0.5f;
            float cz = fz - 0.5f;
    ...
    // Estimate thresholds for 15% low, 60% mid, 25% high
    // Cheap approximation using histogram instead of sorting 1M+ values
    const int BINS = 512;
    int hist[BINS];
    ...
    int highTarget = (int)(0.75f * total); // above this is top 25%
...
static void depthColor(float h, float &r, float &g, float &b)
{
    ...
    if (h <= colorLowCut) {
        ...
        float hue = 270.0f - t * 60.0f; // purple -> blue/cyan
        float H = hue / 60.0f;
        int i = (int)H % 6;
        float f = H - floorf(H);
        float q = 1.0f - f;
        ...
        float hue = 120.0f - t * 120.0f; // green -> yellow -> red
        float H = hue / 60.0f;
        int i = (int)H % 6;
        float f = H - floorf(H);
        float q = 1.0f - f;
        ...
        // Keep mid band centered around green, with slight cyan on lower side
        // and slight yellow on upper side, but mostly green.
        float hue = 180.0f - t * 60.0f; // cyan-green -> green -> yellow-green
        float H = hue / 60.0f;
        int i = (int)H % 6;
        float f = H - floorf(H);
        float q = 1.0f - f;
...
static void drawHUD(int w, int h)
{
    ...
    drawText2D(15, 20, "WASD: Move  Mouse Wheel: Zoom  LMB Hold: Scan  RMB Drag: Rotate  C: Sonar POV  R: Reset");
...
{
    if (camSonarPOV) {
        // Camera attached slightly behind and above the sonar device
        float ax = deg2rad(camAngleX);
        float ay = deg2rad(camAngleY);

        float offsetBack = 4.0f;
        float camX = sonarX - offsetBack * sinf(ay) * cosf(ax);
        float camZ = sonarZ - offsetBack * cosf(ay) * cosf(ax);
        float camY = SONAR_H + 2.0f;

        float lookX = sonarX + sinf(ay) * cosf(ax) * 20.0f;
        float lookZ = sonarZ + cosf(ay) * cosf(ax) * 20.0f;
        float lookY = SONAR_H - 8.0f;

        gluLookAt(camX, camY, camZ, lookX, lookY, lookZ, 0.0f, 1.0f, 0.0f);
    } else {
        // Original orbiting camera but kept corner-ish when zooming out
        float cx = WORLD * 0.5f;
        float cz = WORLD * 0.5f;
        float ax = deg2rad(camAngleX);
        float ay = deg2rad(camAngleY);

        float camX = cx + camDistance * sinf(ay) * cosf(ax);
        float camZ = cz + camDistance * cosf(ay) * cosf(ax);

        // make vertical rise slower than distance so it stays corner-ish
        float camY = 28.0f + camDistance * 0.28f * sinf(ax);

        gluLookAt(camX, camY, camZ, cx, 0.0f, cz, 0.0f, 1.0f, 0.0f);
    }
}
...
static void mouseMotion(int x, int y)
{
    if (rightMouseDown) {
        ...
        camAngleY += dx * 0.25f;     // yaw
        camAngleX += dy * 0.25f;     // pitch
